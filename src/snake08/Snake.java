			...
			while(true) {
				...
				try {
					...
					if(input.available() > 0) {
						...
						// 绘制食物
						SnakeBit foodObj = (SnakeBit) jsonObj.get("food");
						if(food == null) { // 当前界面中还没有食物
							drawSnakeXY(foodObj.getX(), foodObj.getY(), foodObj.getColor());
						} else {
							if(food.getX()!=foodObj.getX() || food.getY()!=foodObj.getY()) { // 说明食物的位置改变了，就是被另一个蛇吃掉了，需要重新绘制
								deleteSnakeXY(food.getX(), food.getY(), food.getColor());
								drawSnakeXY(foodObj.getX(), foodObj.getY(), foodObj.getColor());
								food.setX(foodObj.getX()); // 更新食物对象的位置值
								food.setY(foodObj.getY());							
							}
						}
						...
						if(jsonObj.getBoolean("isAlive")) { // 如果另一条蛇还活着，将之前的蛇擦除掉，在新的位置完全重新绘制
							...
							if(preOtherSnakeObj != null) { // 如果不是第一次绘制，则需要擦除之前的蛇							
								for (int i = 0; i < preOtherSnakeObj.length(); i++) {
									SnakeBit bit = (SnakeBit) preOtherSnakeObj.get(i);
									deleteSnakeXY(bit.getX(), bit.getY(), Color.WHITE);								
								}
							}
							for (int j = 0; j < snakeArr.length(); j++) { // 绘制新的蛇
								SnakeBit bit = (SnakeBit) snakeArr.get(j);
								drawSnakeXY(bit.getX(), bit.getY(), bit.getColor());
							}
							preOtherSnakeObj = snakeArr;												
						} else { // 对方蛇死掉了
							for (int i = 0; i < preOtherSnakeObj.length(); i++) { // 界面中清除掉对方的蛇
								SnakeBit bit = (SnakeBit) preOtherSnakeObj.get(i);
								deleteSnakeXY(bit.getX(), bit.getY(), Color.WHITE);								
							}
						}	
